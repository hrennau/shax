...
(:~
 ...
 : normalized.
 :)
declare function f:normalizeShaxModelNamespaceBindings($elems as element()+)
        as element()+ {
    let $nsmap := f:getShaxModelNamespaceMap($elems)
    return
        for $elem in $elems return f:normalizeShaxModelNamespaceBindingsRC($elem, $nsmap)
};
